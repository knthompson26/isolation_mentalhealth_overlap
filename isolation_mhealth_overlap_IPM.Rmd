...
```{r DZ matrices}
# covariance matrix
covar.dz <- cov(dat.twin.DZ[, selvars_noanx], use = "complete")
# correlation matrix (standardized covariance)
cor.dz <- cor(dat.twin.DZ[, selvars_noanx], use = "complete")
as.data.frame(round(cor.dz, 3))
```
...
# saveRDS(IPM_allvar_noanx_scalarhet_fit, "../../data_full/IPM_allvar_noanx_scalarhet_fit.rds")
IPM_allvar_noanx_scalarhet_fit <- readRDS("../../data_full/IPM_allvar_noanx_scalarhet_fit.rds")
```
...
```{r reformatting to reflect size of r males}
...
```{r save male stacked}
ggsave(stacked_bar_male_resized, file = "/Users/katiethompson/Documents/PhD/LISS-DTP_Louise_and_Tim/Social isolation_genetics_Paper 3/data_analysis/plots/stackedbar_male_resized.png", height = 6, width = 9)
```
...
```{r reformatting to reflect size of r females}
contribution_output.female.reorder_new <- contribution_output.female.formatted %>%
  select(-total, -sex) %>%
  pivot_longer(!`phenotypic correlation` & !direction & !construct & !r, 
               names_to = "Percentage contribution", 
               values_to = "Percent (%)") %>%
  rename(r_new = r) %>%
  mutate(r_labels = labels_female) %>%
  mutate(position = 100) %>%
  mutate_if(is.numeric, ~na_if(., 0)) %>%
  mutate(r = `Percent (%)`*0.01*r_new)
```
...
```{r stacked bar for females}
stacked_bar_female_resized <- ggplot(
  contribution_output.female.reorder_new,
  aes(fill = `Percentage contribution`,
      y = `r`,
      x = `phenotypic correlation`)) + 
  geom_bar(position = "stack", 
...
stacked_bar_female_resized
```

```{r save female stacked resized}
ggsave(stacked_bar_female_resized, file = "/Users/katiethompson/Documents/PhD/LISS-DTP_Louise_and_Tim/Social isolation_genetics_Paper 3/data_analysis/plots/stackedbar_female_resized.png", height = 6, width = 9)
```
