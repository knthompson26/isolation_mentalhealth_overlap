...
# MZ
cor(dat.twin.MZ[, selvars_norm_reg], use = "complete")

# DZ
cor(dat.twin.DZ[, selvars_norm_reg], use = "complete")
```
...
mxEval(MZ.Rwithin, PhCSrphFit)  # correlation between latent factors - shoudl be 0
...
mxEval(MZ.StandFact, PhCSrphFit)   # first 5 are from factor 1, and the second five are the last five from factor 2
mxEval(MZ.StandFact61, PhCSrphFit)  # time two variables factor 1 for variables 6-10
mxEval(MZ.StandFact71, PhCSrphFit)  # small - Anxiety age 18 on factor 1
mxEval(MZ.StandFact81, PhCSrphFit)
mxEval(MZ.StandFact91, PhCSrphFit)
mxEval(MZ.StandFact101, PhCSrphFit)
...
We allow heterogeneity paths for variable 2, so we specify separate ACEc (common) and ACEs (specific) matrices in males and females but only allow the labels for variable 2 to vary by sex. All the other labels are the same in males and females - thus constraining these to be equal. 
...
```
###### Output for chosen model SO FAR
...
```{r Ac heat map}
# reformat % matrix
contribution_output_Ac_arrange1 <- contribution_output %>%
  filter(sex == "Male") %>%
  separate(`phenotypic correlation`, c("X1", "X2")) %>%
  select(X1, X2, value = `Ac % contribution`) %>%
  arrange(variable1)

contribution_output_Ac_arrange2 <- contribution_output_Ac_arrange1 %>%
  mutate(X1_new = X2) %>%
  mutate(X2_new = X1) %>%
  select(X1 = X1_new,
         X2 = X2_new,
...
contribution_output_Ac <- rbind(contribution_output_Ac_arrange1, contribution_output_Ac_arrange2, extra) %>%
  arrange(X1)

# heat map for Ac
heat_map_Ac <- ggplot(contribution_output_Ac, aes(X1, X2, fill = value)) +
 geom_tile(color = "white") +
...
```{r Cc heat map}
# reformat % matrix
contribution_output_Cc_arrange1 <- contribution_output %>%
  filter(sex == "Male") %>%
  separate(`phenotypic correlation`, c("X1", "X2")) %>%
  select(X1, X2, value = `Cc % contribution`) %>%
  arrange(variable1)

contribution_output_Cc_arrange2 <- contribution_output_Cc_arrange1 %>%
  mutate(X1_new = X2) %>%
  mutate(X2_new = X1) %>%
  select(X1 = X1_new,
         X2 = X2_new,
...
contribution_output_Cc <- rbind(contribution_output_Cc_arrange1, contribution_output_Cc_arrange2, extra) %>%
  arrange(X1)

# heat map for Cc
heat_map_Cc <- ggplot(contribution_output_Cc, aes(X1, X2, fill = value)) +
 geom_tile(color = "white") +
 scale_fill_gradientn(colours = colour_purple,
   limit = c(0,100), space = "Lab",
...
```{r Ec heat map}
# reformat % matrix
contribution_output_Ec_arrange1 <- contribution_output %>%
  filter(sex == "Male") %>%
  separate(`phenotypic correlation`, c("X1", "X2")) %>%
  select(X1, X2, value = `Ec % contribution`) %>%
  arrange(variable1)

contribution_output_Ec_arrange2 <- contribution_output_Ec_arrange1 %>%
  mutate(X1_new = X2) %>%
  mutate(X2_new = X1) %>%
  select(X1 = X1_new,
         X2 = X2_new,
...
contribution_output_Ec <- rbind(contribution_output_Ec_arrange1, contribution_output_Ec_arrange2, extra) %>%
  arrange(X1)

# heat map for Ec
heat_map_Ec <- ggplot(contribution_output_Ec, aes(X1, X2, fill = value)) +
 geom_tile(color = "white") +
 scale_fill_gradientn(colours = colour_purple,
   limit = c(0,100), space = "Lab",
...
contribution <- ggpubr::ggarrange(heat_map_Ac, heat_map_Cc, heat_map_Ec, nrow = 3, labels = c('a', 'b', 'c'))

ggsave(contribution, file = "../../../preliminary_results/contribution_heatmap.jpeg", height = 10, width = 6)
```

## Stacked bar

```{r stacked bar chart for contribution %}
contribution_output_arrange3 <- contribution_output %>%
  filter(sex == "Male") %>%
  select(-total, -sex) %>%
  pivot_longer(!`phenotypic correlation`, names_to = "contribution", values_to = "percent")
  
  
stacked_bar <- ggplot(contribution_output_arrange3, aes(fill=contribution, y=percent, x=`phenotypic correlation`)) + 
  geom_bar(position="stack", stat="identity") +
  coord_flip() +
  scale_fill_manual(values = c("#7B287D", "#7067CF", "#CBF3D2"))

stacked_bar

# stacked bar chart for the phenotypic correlations that were above 0.15
contribution_output_arrange4 <- contribution_output %>%
  filter(sex == "Male",
         `phenotypic correlation` != "Isolation12_Psychosis12",
         `phenotypic correlation` != "Isolation12_Depression18",
         `phenotypic correlation` != "Depression12_Conduct18",
         `phenotypic correlation` != "Conduct12_Depression18",
         `phenotypic correlation` != "Psychosis12_Isolation18",
         `phenotypic correlation` != "Psychosis12_Depression18",
         `phenotypic correlation` != "Psychosis12_Conduct18"
  ) %>%
  select(-total, -sex) %>%
  pivot_longer(!`phenotypic correlation`, names_to = "contribution", values_to = "percent")

condensed_contribution_stacked_bar <- ggplot(contribution_output_arrange4, 
                                 aes(fill=contribution, y=percent, x=`phenotypic correlation`)) + 
  geom_bar(position="stack", stat="identity") +
  coord_flip() +
  scale_fill_manual(values = c("#7B287D", "#7067CF", "#CBF3D2"))

ggsave(condensed_contribution_stacked_bar, file = "../../../preliminary_results/condensed_contribution_stackedbar.png", height = 8, width = 10)
```
