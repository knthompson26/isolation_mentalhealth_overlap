...
```{r check for sex as a covariate - conduct}
# regression analyses in elder twin members (MZ and DZ)
rcone12_sex <- summary(lm(conec12 ~ sex, data = dat.twin))
rcone18_sex <- summary(lm(cdsxe18 ~ sex, data = dat.twin))
# regression analyses in younger twin members (MZ and DZ)
rcony12_sex <- summary(lm(conyc12 ~ sex, data = dat.twin))
rcony18_sex <- summary(lm(cdsxy18 ~ sex, data = dat.twin))
```

```{r check for sex as a covariate - psychosis}
# regression analyses in elder twin members (MZ and DZ)
rpsye12_sex <- summary(lm(psysympe12 ~ sex, data = dat.twin))
rpsye18_sex <- summary(lm(psyexpe18 ~ sex, data = dat.twin))
# regression analyses in younger twin members (MZ and DZ)
rpsyy12_sex <- summary(lm(psysympy12 ~ sex, data = dat.twin))
rpsyy18_sex <- summary(lm(psyexpy18 ~ sex, data = dat.twin))
```

Therefore, sex significantly predicts isolation at age 12, all anxiety variables, depression at age 18, all conduct variables, and none of the psychosis variables. 
...
* Bivariate models for cross-phenotype within each age
...
**The bivariate model wouldn't run using raw estimates - only normalised.** 
...
# run ACE
AceFit_depression1218 <- mxTryHard(AceModel, intervals = TRUE)
```
...
From the bivariate models (normalised):
Depression	12	h2	0.189	
Depression	18	h2	0.320
